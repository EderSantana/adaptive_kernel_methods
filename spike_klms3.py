...
class SpikeKLMS(BaseEstimator, TransformerMixin):
    ...
    growing_criterion: "dense" | "novelty" | "quantized" | "s_quantized" |"surprise"
        Default: "dense:"
    ...
    def transform(self, X, Z):
        ...
        for i in xrange(len(Z)):
            print i
            Z_out[i] = np.dot(self.coeff_, \
                       self._get_kernel([X[k] for k in self.centerIndex_],Z[i]))
    ...
    def fit_transform(self, X, d, err=None):
        ...
        self.X_transformed_ = self.transform(X,X)

        return self.X_transformed_
 
    def _trainNet(self, X, newX, err, k):
        ...
        if self.coeff_.shape[0] == 0:
            ...
            #===========================
                ...
                distanc = slash.ppMCIdistance([X[_idx] for _idx in self.centerIndex_], newX, \
            ...
            #===========================
                ...
                # Sample the quanta from a multinomial distribution
					mult_prob = np.exp(distanc/np.var(distanc))
					mult_prob = mult_prob/np.sum(mult_prob)
					mult_sample = find( np.random.multinomial(1, mult_prob) == 1 )
					self.coeff_[mult_sample] += self.learning_rate * \
					                            self._loss_derivative(err)
                else:
                    self.coeff_ = np.append(self.coeff_, self.learning_rate *
                                               self._loss_derivative(err))
            ...
            #  RAKESH QUANTIZATION
            #===========================
            elif self.growing_criterion == "s_quantized2":
                inners = slash.ppMCI([X[_idx] for _idx in self.centerIndex_], \
                                   X[k], ksize = self.ksize)
                if np.max(inners) >= self.growing_criterion[0]:
                    mult_prob = inners / np.sum(inners)
                    mult_sample = find( np.random.multinomial(1, mult_prob) == 1 )                          
                    self.coeff_[mult_sample] += self.learning_rate * \
                                                self._loss_derivative(err)
                else:
